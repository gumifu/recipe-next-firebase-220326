...
export default function Home() {
  ...
  useEffect(() => {
    async function getRecipes() {
      const recipeCollection = collection(database, 'recipes');//recipesにアクセス
      const recipeSnapshot = await getDocs(recipeCollection);//データを格納
      const recipes = recipeSnapshot.docs.map((doc) => {//mapで広げる
        const data = doc.data();
        data.id = doc.id;
        return data
      })
